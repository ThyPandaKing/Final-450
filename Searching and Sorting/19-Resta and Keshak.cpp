...
long long s1[1 << 10][1 << 10], s2[1 << 10][1 << 10], ar1[1 << 10][1 << 10],
    ar2[1 << 10][1 << 10], pw1[1 << 22], pw2[1 << 22], n2, m2;
long long l, r;
long long n1, m1;
...
bool solve(int X) {
    ...
    for (int i = 0; i + X <= n1; i++)
        for (int j = 0; j + X <= m1; j++) have.push_back(get1(i, j, X));
...
int main() {
    ...
    pw1[0] = 1;
    for (int i = 1; i <= 2000000; i++) pw1[i] = pw1[i - 1] * 173;

    pw2[0] = 1;
    for (int i = 1; i <= 2000000; i++) pw2[i] = pw2[i - 1] * 137;

    cin >> n1 >> m1;
    for (int i = 1; i <= n1; i++)
        for (int j = 1; j <= m1; j++) cin >> ar1[i][j];

    cin >> n2 >> m2;
    for (int i = 1; i <= n2; i++)
        for (int j = 1; j <= m2; j++) cin >> ar2[i][j];
